...
\begin{abstract}
    This report describes the design, implementation, and verification of a 4‑bit Arithmetic Logic Unit in Verilog using a hierarchical, modular approach.  
    Core combinational submodules implement logic gates, addition, subtraction, multiplication, and division, while a finite‑state‑machine controller sequences
    multi‑cycle operations. A generic Verilog testbench exercises all opcodes and operands with GTKWave timing diagrams revealing a clear, operation‑specific
    patterns such as a rising staircase remainder in division and half‑wave mirror symmetry in multiplication, confirming correct synchronization between data
    paths and control logic. The result is a fully functional, easily extensible ALU whose design balances clarity, reusability, and testability, laying the 
...
The testing algorithm shown in section \textbf{Sub-Controller Testing} iterates the two main register that control the inputs of each operation. They can be described and shown in the
figure below:
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        IN1 & 8{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of one complete input cycle}
    \label{fig:REG2_time}
\end{figure}

Diagram \ref{fig:REG2_time} shows how \texttt{REG2} is affected by \texttt{REG1}. The generic tests follow a pattern while iterating that allow for every possible combination of inputs to be passed
into the modules being tested; in this case, the ALU controller and its sub-controllers. For \texttt{REG1}, it starts at a value of 0 and increments by one after completing a full cycle
of \texttt{REG2}, so that means that every change in input from \texttt{REG1} has an equivalent pattern shown in the timing diagram \ref{fig:REG2_time}.

\subsubsection*{Control Signal Timing}
...
Diagram \ref{fig:ctrl_time} illustrates the timing of the control signals that accompany every individual change in the input. Importantly, although Figure \ref{fig:REG2_time} depicts a 
complete input cycle of REG2, \textit{each} increment or update in REG2 triggers the control signal sequence. Thus, every time IN2 is updated (e.g., when IN2 changes from 0 to 1, or from
1 to 2, and so on), the corresponding control signals (RST, START, and DONE) are activated, ensuring that the ALU controller processes each new input reliably and synchronously with the 
clock signal.
...
Furthermore, since every change in input results in the control signals in diagram \ref{fig:ctrl_time}, every input that is changed in the timing diagram \ref{fig:REG2_time} has these
signals working in the background.
...
The divider controller is the fourth of the sub-controllers developed and is responsible for performing the division operation. The design and implementation of the division was the most complex of the four operations, as it
required multiple intermediary steps to be performed before each step could be done. To avoid conflating the main module of the division with too many operations, it was integral to separate the division into 3 parts:
\begin{itemize}
    \item \textbf{Signal Decomposition}: The first intermediary step performed in the division logic that is used to decompose an input signal into its most significant components.
    \item \textbf{Signal Alignment}: The second intermediary step performed in the division logic that is used to align an input to another input based on the MSB of its second input.
    \item \textbf{Divider Core}: The main method of the divider which performs subtraction operation and assigns the output registers.
\end{itemize}

\subsubsection*{Divider Core}
Most of the changes made to the division operation from the previous report is the addition of the \textit{divider core} module. It is a refactored version of previous combinational logic that has been isolated into its own module.
This is in order to promote a modular design and allow for the division module to be used across multiple logic blocks. Similar to how the \textit{multiplier core} was designed, the \textit{divider core} performs its operations on a
whole input rather than a single bit for one clock cycle.

The \textit{divider core} uses both the \textit{signal decomposition} and \textit{signal alignment} as well as a \textit{Full Subtractor} and a few \textit{comparators} to perform and complete each step of the dicision process.
The \textit{divider core} is shown below:
\begin{lstlisting}[language=Verilog]
...
The \textit{divider core} functions by first extracting the most significant bits using the \textit{signal decomposition} and then aligning the inputs based on the position of that MSB in the \textit{signal alignment} step. From
there, the aligned divisor is subtracted from the dividend using a \textit{Full Subtractor}, where a quotient is then placed based on the position determined by the \textit{signal decomposition} step using a \textit{nBit Shifter}.
Upon completing its operation, the \textit{divider core} issues a few \textit{comparators} checks to determine some key features of the inputs being calculated. The \textit{comparators} are used by the \textit{divider controller} 
to set rules for the division operation and determine if the operation is valid or not. The \textit{comparators} used in the \textit{divider core} are:
\begin{itemize}
    \item \textbf{First Equal To Comparator}: The first comparator featured in the final checks of the \textit{divider core} is used to determine if a divisor is zero. This is used by the \textit{divider controller} to determine if
        the division operation performed is valid or not. If the divisor is zero, the division operation fails and the output registers are not updated.
    \item \textbf{Less Than Comparator}: The second comparator featured is used to determine if the dividend is less than the divider. This is used by the \textit{divider controller} to determine if the division operation is valid or not.
        As opposed to the zero check, if this comparator fails, then the divisor does not fit into the dividend and a remainder can be set.
    \item \textbf{Second Equal To Comparator}: The third comparator featured is used to determine if there is a final borrow from the subtraction operation. This is used by the \textit{divider controller} to determine if another step is needed.
        So long as this comparator is true, the division operation will continue in a loop.
\end{itemize}

\subsubsection*{Divider Controller FSM}
The FSM for the divider controller is identical to the one used for the multiplier controller, with only a slight difference in the \texttt{INIT} state. In the \texttt{INIT} state, the \textit{divider core} needs to have its quotient
and remainder initialized based on the current inputs. Although this step is not strictly necessary the first time the division process works, it helps to ensure that the remaining steps are performed and initialized correctly based on
a step counter passed down from the \textit{divider controller} to the \textit{divider core}.

Once the division operation is finished, a \texttt{DONE} state is entered, where the output registers are updated and the FSM is reset to the \texttt{IDLE} state. From here, the \textit{divider controller} takes back control and the ALU 
controller can proceed to the next operation.

\section{Waveform Analysis}
Every operation performed by the ALU produces an output observable in waveform form. These waveforms are tied to the ALU controller’s output registers, with each register reflecting part of the computed result. The three key output 
registers are:
\begin{itemize}
    \item \textbf{p\_high\_latch}: The high portion of the output register that is used to store the final result of operations with sectional outputs.
    \item \textbf{p\_low\_latch}: The low portion of the output register that is used to store the final result of most standard operations.
    \item \textbf{flag\_latch}: The flag register that is used to represent any single bit results of the operations. 
\end{itemize}

Every output register utilizes the latch discussed in the subsection \textbf{Latch Implementation} in order to create a stable output wave to analyze.
...
\subsection{Addition Waveform}
The first waveform discussed is the simplest to understand, since it follows a strict pattern that can be easily analyzed. Because of the method used to iterate through the operations, a distinct pattern can be formed that shows a progressive growth
of two output signals. The output signals used in the addition operation are as follows:
\begin{itemize}
    \item \textbf{flag\_latch}: Represents any carry bits produced by the addition operation.
    \item \textbf{p\_low\_latch}: Represents the output of the addition operation.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/addition_waveform.png}
    \caption{Addition waveform from the ALU from GTKWave}
    \label{fig:add_wave}
\end{figure}

\subsubsection*{Addition Timing Diagram}
The waveform can be decomposed into sectional outputs, where each section represents a single bit, such as what is shown in figure \ref{fig:add_wave}. As the addition operation produces a single-segment result, the \texttt{p\_high\_latch} remains inactive. 
Only \texttt{p\_low\_latch} is engaged, representing a 4-bit result, while \texttt{flag\_latch} captures overflow through the carry bit. The output of \texttt{p\_low\_latch} is observed bit-wise, labeled \texttt{OUT1} through \texttt{OUT4}, with bit 
significance increasing from LSB to MSB.
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        CARRY & 16L \\
        OUT4 & 7L 8H 1L \\
        OUT3 & 3L 4H 4L 4H 1L\\
        OUT2 & 1L 3{2H2L} 2H 1L \\
        OUT1 & 8{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the addition operation, showing the first complete input cycle}
\end{figure}

The output of the addition operation follows the same general pattern for its \texttt{OUT} signals with the only register showing any changes over time being the \texttt{CARRY} signal. At the end of the first complete input cycle, the \texttt{CARRY} signal
briefly rises, increasing its duration with each successive addition of larger numbers.

For example, the next input cycle following the logic adds \texttt{0001} instead of \texttt{0000} to all possible combinations of \texttt{in2}. This results in an output shown as:
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        OUT1 & 8{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the addition operation, showing the second complete input cycle}
\end{figure}

The next input cycle would then be \texttt{0010} instead of \texttt{0001}, and results in the following output:
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        CARRY & 13L 2H 1L \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/subtraction_waveform.png}
    \caption{Subtraction waveform from the ALU}
    \label{fig:sub_wave}
\end{figure}

\subsubsection*{Subtraction Timing Diagram}
The subtraction operation is decomposed identically as the addition operation was, with the same output signals being used. However, the \texttt{CARRY} signal is now replaced with a \texttt{BORROW} signal, which is used to represent any borrow bits
produced by the subtraction operation. Furthermore, the subtraction operation experiences a decay in its \texttt{BORROW} signal instead of a growth like the addition operation.
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        OUT1 & 8{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the subtraction operation, showing the first complete input cycle}
\end{figure}

Negative results are produced via two’s‑complement: the output bits are inverted, then the borrow bit is asserted to complete the +1 step. Essentially, the outputs are the opposite from that seen in the addition operation. So instead of the output being 
\texttt{0000} for the first input cycle, it is now \texttt{1111} with a borrow of \texttt{1}. For the remaining cycles, the output is the same, but the borrow signal decays in length.

Using the same example as before, the output from the next input cycle is:
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        BORROW & 14H 2L \\
        ...
        OUT1 & 8{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the subtraction operation, showing the second complete input cycle}
\end{figure}
...
\subsection{Multiplication Waveform}
...
\begin{itemize}
    \item \textbf{p\_high\_latch}: Represents the high output bits of the multiplication operation.
    \item \textbf{p\_low\_latch}: Represents the low output bits of the multiplication operation.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/multiplication_waveform.png}
    \caption{Multiplication waveform from the ALU}
    \label{fig:mul_wave}
\end{figure}

\subsubsection*{Multiplication Timing Diagram}
The waveform can be decomposed into sectional outputs, similar to how they are demonstrated before, and is shown in figure \ref{fig:mul_wave}. Since the output representation has changed, the \texttt{p\_high\_latch} becomes active and the
\texttt{flag\_latch} is now disabled. Both high and low latches capture a pre-designated 4-bit result, with the latch method remaining consistent. Both high and low outputs are also observed bit-wise and are labeled \texttt{OUT1} through 
\texttt{OUT4} for both outputs respectively. Furthermore, their bit significance increases the same way as before: by increments of one for inputs one and two.
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        and so on... \\
        OUT1 HIGH & 23L 8H 1L \\
        OUT4 LOW & 7L 1{8H} 3L 1L 1{4H4L} 4H 1L \\
        OUT3 LOW & 3L 1{4H4L} 4H 2L 3{2H2L} 2H 1L \\
        OUT2 LOW & 1L 3{2H2L} 2H 1L 8{HL} \\
        OUT1 LOW & 8{HL} 16L\\
    \end{tikztimingtable}
    \caption{Timing diagram of the multiplication operation, showing the next two complete input cycles after the first}
\end{figure}

The timing diagram above starts with an input of \texttt{0001} instead of \texttt{0000} since the output produces no significant information; the output is 0. Furthermore, it is observed that the waveform is being shifted up the output register.
This is due to the method used to solve the multiplication in binary, where the output is shifted based on the bits of the multiplicand. However, this pattern does not remain consistent across the entire multiplication operation as shown below:
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        and so on... \\
        OUT2 HIGH & 8L 3H 11L 8H 2L \\
        OUT1 HIGH &  5L 3H 11L 3H 5L 3H 2L \\
        OUT4 LOW & 2L 3H 2L 3H 3L 2H 4{2L2H} 1L \\
        OUT3 LOW & L H L 2H L H 2L H L 2H L H L 8{HL} \\
        OUT2 LOW & 3{2H2L} 2H 18L \\
        OUT1 LOW & 8{HL} 16L\\
    \end{tikztimingtable}
    \caption{Timing diagram of the multiplication operation, showing the next two complete input cycles after the previous}
\end{figure}

As the multiplication operation continues, the upper output registers are utilized more frequently and present a growth similar to that seen in the addition operation. The waveforms slowly grow into the pattern observed in the input cycles in these upper bits, 
like how \ref{fig:REG2_time} shows.

\subsubsection*{Mirror Symmetry Effect}
A key observation of the lower output registers in the multiplication operation timing diagrams is that the pattern between the multiplicand of any number $M$ and $16-M$ are bit-wise complements of one another. For this to occur, \texttt{M} would need to be 
greater than \texttt{0} and less than the \texttt{width} of the bits; here the \texttt{width} is assigned to \texttt{4}. In essence, the output of one half of the waveform is a mirror image of its other half. This is a property of \textbf{half-wave symmetry}, where the output of the waveform is a mirror image 
of itself across a vertical line drawn through the midpoint of a waveform. This mirror image can be described as the two's complement of the first half of the waveform. 

In the waveform shown in \ref{fig:mul_wave}, the mirror can be observed at the midpoint \texttt{8}.

\subsection{Division Waveform}
...
\begin{itemize}
    \item \textbf{p\_high\_latch}: Represents the quotient of the division operation.
    \item \textbf{p\_low\_latch}: Represents the remainder of the division operation.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/division_waveform.png}
    \caption{Division waveform from the ALU}
    \label{fig:div_wave}
\end{figure}

\subsubsection*{Division Timing Diagram}
The waveform is decomposed into similar sectional outputs as before, with the \texttt{p\_high\_latch} representing the quotient calculated and the \texttt{p\_low\_latch} representing any
remainder. Thus, both the  \texttt{p\_high\_latch} and the \texttt{p\_low\_latch} are still enabled during this operation. The same method of incrementing is applied here as it was done 
previously.
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        SAMPLE & 16{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the division operation, showing the next two complete input cycles after the first.}
\end{figure}

The diagram begins at the second input cycle, when \texttt{in1} is \texttt{0001} to avoid dividing numbers by \texttt{0}. Furthermore, it shows both the remaining bits and the quotient 
from the operations \texttt{0001} and \texttt{0010}, capturing their results as \texttt{in2} is iterated upon. The pattern showcased maintains a consistent and predictable flow, similarly
seen in the addition operation.
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        and so on... \\
        OUT3 HIGH & 17L H 14L \\
        OUT2 HIGH & L H 16L H 13L \\
        OUT1 HIGH & L 3H 15L 2H 11L \\
        OUT4 LOW & 32L \\
        OUT3 LOW & 21L 11H \\
        OUT2 LOW & 4L 12H 16L\\
        OUT1 LOW & 2L H L 12H 3L H 12L \\
        SAMPLE & 16{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the division operation, showing the next two complete input cycles after the previous.}
\end{figure}

Every iteration produces a step on \texttt{p\_high\_latch} to the quotient and updates \texttt{p\_low\_latch} to the new remainder, which remains constant for the following shift phase. 
This alternating pulse‑and‑hold pattern yields a rising‑staircase on the remainder bus, since successive subtractions accumulate interleaved with single‑cycle high pulses on the quotient
bus. When the partial remainder falls below the divisor, the quotient bit is \texttt{0}, and the \texttt{p\_high\_latch} remains low. Furthermore, the \texttt{p\_low\_latch} shifts the 
previous remainder forward. Only when the remainder is greater than the divisor is when a non-zero stair-step occurs.

\section{Conclusion}
Throughout the project, a successful 4‑bit ALU in Verilog is built by applying a clean, hierarchical design methodology. Simple combinational cores for each arithmetic and logic function, 
driven by a central FSM‑based controller allowed for a robust ALU with greater capabilities as its iterations were expanded on. Further separating core modules from their subcontrollers 
made the codebase more maintainable and simplified timing‑mismatches encountered while debugging. Extensive testing using testbenchs helped to cover every possibility in the ALU and GTKWave
waveform analysis demonstrated that every operation behaves as expected. The addition and subtraction operations exhibited a growth/decay pattern in their carry/borrow flags, multiplication
operations showcase a form of half‑wave symmetry, and division operations producing a distinctive sawtooth-esq remainder pattern.

Looking ahead, there are several promising avenues for enhancement:
\begin{itemize}
  \item \textbf{Signed‑Number Support:} Extend addition, subtraction, and comparison logic to handle two’s‑complement operands.  
  \item \textbf{Floating‑Point Unit:} Integrate IEEE‑754 compliant modules for real‑number arithmetic.  
  \item \textbf{FPGA Synthesis \& Timing Closure:} Port the design to an FPGA platform, optimize for setup/hold times, and verify performance under real‑world clock constraints.  
  \item \textbf{Performance Optimizations:} Explore look‑ahead carry adders, non‑restoring division algorithms, or pipelined control to reduce cycle counts.  
\end{itemize}
