...
Negative results are produced via two’s‑complement: the output bits are inverted, then the borrow bit is asserted to complete the +1 step. This is done by taking the 2's complement of the output signal, which is done by inverting the output signal and 
adding 1 to it. Essentially, the outputs are inverted from that seen in the addition operation. So instead of the output being \texttt{0000} for the first input cycle, it is now \texttt{1111} with a borrow of \texttt{1}. For the remaining cycles, the 
output is the same, but the borrow signal decays in length.
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        OUT1 LOW & 8{HL} 16L\\
    \end{tikztimingtable}
    \caption{Timing diagram of the multiplication operation, showing the first two complete input cycles}
\end{figure}
