...
The third waveform discussed is the multiplication operation. This operation differs in its output signals and are not as cleanly represented like how addition or subtraction are however, there still exists a pattern within the waveform. The output
for the multiplication operation is split into two outputs:
\begin{itemize}
...
This is to keep the size of bits similar across the system and is due to the incrreased size of the output produced by multiplying two numbers. 
...
\texttt{flag\_latch} becomes inactive. Both high and low latches capture a designated 4-bit result, with the latch method remaining consistent. Both high and low outputs are also observed bit-wise and are labeled \texttt{OUT1} through 
\texttt{OUT4}, with bit significance increasing the same way as before.
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        and so on... \\
...
From the first two input cycles, it is observed that the waveform is being shifted up the output register. This is due to the nature of the multiplication operation, where the output is shifted based on the bits of the multiplicand. However,
this pattern does not remain consistent across the entire multiplication operation:
\begin{figure}[H]
    \centering
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        and so on... \\
...
% Discuss how the wavefrom progressively grows; mention where we start the signal: e.g. when in1 is 1, and in2 goes through its inputs 
...
\begin{figure}[H]
    ...
    \begin{tikztimingtable}[timing/slope=0, xscale=1.2]
        ...
        SAMPLE & 16{HL} \\
    \end{tikztimingtable}
    \caption{Timing diagram of the division operation, showing the first two complete input cycles}
\end{figure}
