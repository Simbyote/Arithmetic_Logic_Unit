...
% Enhance the section by potentially adding how clock signals affect overall system performance or maybe the hardware implications?
% The runtime of the ALU is around 300,000 nanoseconds, which is 1/3 of a millisecond. This is a very small amount of time, but it is still
% significant in the context of a computer system. 
...
% Further enhancements could include comparisons between my design decisions and those done professionally. This would add more context 
% to my design decisions and show how they are similar or different to those done in the industry.
...
Furthermore, since the ALU controller was split into two function, it would not be ideal for both the ALU controller and the sub-controllers to be sequential. The interactions between each controllers
cycles would create more complicated timing mismatches, which in turn would require more complex logic to manage. So it was declared that the ALU controller would solely be combinational, while still
housing the timing inputs to feed into the sub-controllers; those of which did function on the timing inputs. This design decision allowed for the separation of concerns between the different controllers
developed for the ALU and allowed for a more modular design.
