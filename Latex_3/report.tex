...
were changed to accommodate the new design of the ALU. Each operation now included a state table and an FSM to determine where in the calculation process the operation was. This in turn
added additional clarity to the design of the ALU, and furthermore, allowed the operations to be performed in a sequential manner rather than combinational. It is important to note that
this is in accordance to how real hardware operates, as hardware takes time to produce results.

\subsection{Sub-Controller Operation}
Each sub-controller runs in a sequential manner. The sub-controller is activated by the ALU controller, which then runs through its FSM to perform the operation. The FSM is brokem down into
...
The Adder controller is the first of the sub-controllers developed and is responsible for performing the addition operation. It consists of its state table, FSM, and the instantiation of the
\textit{Adder Core} unit. The adder controller uses 4 of the states discussed above: \textit{INIT}, \textit{STEP}, \textit{LOAD}, and \textit{DONE}. It then moves through the FSM to perform the addition
sequentially, utilizing the \textit{Adder Core} unit to perform the addition operation per clcok cycle. The \textit{Adder Core} unit performs a single step of the addition computation, which is adding
2 bits together of an input number then adding a carry bit if necessary. The \textit{Adder Core} then outputs the result of the addition and the carry bit if there is one to be used in the next operation,
saved by the addition controller.
